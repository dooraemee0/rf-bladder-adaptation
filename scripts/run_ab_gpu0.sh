#!/usr/bin/env bash
# ==========================================================================
# [GPU 0]  DER++ ablation:  alpha = 0  (dark experience 제거)
#
#   L = L_y(f(x_d), y_d) + 0 * ||f(x_c) - f_old(x_c)||^2 + 0.5 * L_y(f(x_c), y_c)
#
#   -> clinical ground-truth replay 만 남음 (개념적으로 rehearsal 에 가까움)
#
# GPU 1 스크립트(run_ab_gpu1.sh)와 동시에 실행 가능.
...
GPU=0
SEEDS="1,2,3,42"
SAVE_DIR="./results_derpp_ablation/alpha0"
...
export ABL_CACHE_DIR="./_prep_cache_alpha0"

mkdir -p logs "${SAVE_DIR}"
echo "[GPU0 START] alpha=0 (no dark experience)  $(date)"

CUDA_VISIBLE_DEVICES="${GPU}" python -u ablation_A_adaptation.py \
  --ckpt "${CKPT}" --test_idx "${TEST_IDX}" \
  --strategies der++ \
  --seeds "${SEEDS}" --epochs 150 --lr 1e-3 \
  --der_alpha 0.0 --der_beta 0.5 \
  --save_dir "${SAVE_DIR}" \
  2>&1 | tee "logs/derpp_alpha0.log"
